...
def test_is_operational_only_for_active_state() -> None:
    active = Account(id=1, msisdn=1, session_ciphertext='x', state=AccountState.ACTIVE)
    standby = Account(id=2, msisdn=2, session_ciphertext='x', state=AccountState.STANDBY)
...
def test_is_rate_limited_false_when_not_set() -> None:
    account = Account(id=1, msisdn=1, session_ciphertext='x')
