...
class SessionManager:
    ...
    def from_pyrogram_session_file(cls, file: str) -> SessionManager:
        try:
            ...
            msg = 'Invalid Pyrogram session file'
    ...
    def from_telethon_file(cls, file: str, do_login: bool = False) -> SessionManager:
        try:
            ...
            msg = 'Invalid Telethon session file'
            raise ValueError(msg) from exc
    ...
    def telethon_file(self, file: str):
        ...
        conn.execute(
            ...
            (
                self.session.dc_id,
                '.'.join(str(ipaddress.ip_address(self.session.server_address.packed)).split(',')),
                self.session.port,
                self.session.auth_key,
                None,
            ),
