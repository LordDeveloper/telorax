...
class AccountService:
    ...
    async def get_account(self, account_id: int) -> AccountSummaryDTO | None:
        async with SQLAlchemyUnitOfWork(self._session_factory) as unit_of_work:
            account = await unit_of_work.telegram_accounts.get_by_id(account_id)
    ...
    async def list_operational(
        ...
        *,
        ...
        async with SQLAlchemyUnitOfWork(self._session_factory) as unit_of_work:
            accounts = await unit_of_work.telegram_accounts.list_operational(
                limit=limit,
...
def _to_summary(account: TelegramAccount) -> AccountSummaryDTO:
